...
?><?php
$sql = "SELECT s.*, u.avatar FROM status AS s LEFT JOIN users AS u ON u.username = s.author WHERE (s.account_name ='$u' AND s.type='a') OR (s.account_name='$u' AND s.type='c') ORDER BY s.postdate DESC LIMIT 20";
...
// GATHER UP ANY STATUS REPLIES
    $sql2 = "SELECT s.*, u.avatar FROM status AS s LEFT JOIN users AS u ON u.username = s.author WHERE s.osid = '$statusid' AND s.type='b' ORDER BY postdate ASC";
    $status_replies = "";
    ...
    //
            ...
            $status_replies .= '<div id="reply_'.$statusreplyid.'" class="reply_boxes"><div><b>Reply by <a href="user.php?u='.$replyauthor.'">'.$replyauthor.'</a> '.$replypostdate.':</b> '.$replyDeleteButton.'<br/>' .$user_image2 . ' <br />'.$replydata.'</div></div>';
        }
    }
